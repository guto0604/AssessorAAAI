...
from core.investment_case_builder.prompts import build_data_relevance_prompts
...
class DataRelevanceAgent(BaseInvestmentCaseAgent):
    ...
    def _build_fallback_payload(self, *, master_context: dict, advisor_prompt: str, additional_notes: str, tone_focus: str) -> dict:
        ...
        gaps = []
        if not holdings:
            gaps.append({"gap": "Carteira atual não possui posições disponíveis na base local.", "impact": "Reduz profundidade do diagnóstico.", "severity": "high"})
        if not client_info.get("Perfil_Suitability"):
            gaps.append({"gap": "Perfil de suitability ausente.", "impact": "Aumenta incerteza na revisão de risco.", "severity": "high"})
        if not client_info.get("Rentabilidade_12_meses"):
            gaps.append({"gap": "Rentabilidade de 12 meses ausente ou não preenchida.", "impact": "Limita comparação de eficiência.", "severity": "medium"})
        ...
        return {
            ...
            "ignored_data": {
                "fields": [item.get("category") for item in ignored_categories],
                "sections": ["allocation_by_category"] if ignored_categories else [],
    ...
    def run(self, *, master_context: dict, advisor_prompt: str, additional_notes: str = "", tone_focus: str = "") -> AgentResult:
        ...
        system_prompt, user_prompt = build_data_relevance_prompts(
            advisor_prompt=advisor_prompt,
            additional_notes=additional_notes,
            tone_focus=tone_focus,
            master_context=master_context,
            heuristic_baseline=fallback,
        )
        ...
        payload = llm_payload if isinstance(llm_payload, dict) and llm_payload.get("selected_context") else fallback
