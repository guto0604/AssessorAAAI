...
from core.investment_case_builder.prompts import build_diagnosis_prompts
...
class PortfolioDiagnosisAgent(BaseInvestmentCaseAgent):
    ...
    def _build_fallback_diagnosis(self, *, case_state: dict) -> dict:
        ...
        return {
            "current_state": {
                "portfolio_snapshot": client_summary,
                "allocation_summary": categories,
                "liquidity_summary": {"dominant_liquidity": liquidity_profile},
                "risk_profile_read": {"profile": client_summary.get("perfil_suitability")},
                "concentration_summary": {
                    "top_category": top_category,
                    "top_category_pct": concentration_pct,
    ...
    def run(self, *, case_state: dict) -> AgentResult:
        fallback = self._build_fallback_diagnosis(case_state=case_state)
        system_prompt, user_prompt = build_diagnosis_prompts(case_state=case_state, heuristic_baseline=fallback)
